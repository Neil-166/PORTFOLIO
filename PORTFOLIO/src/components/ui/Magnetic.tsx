...
export function Magnetic({ children }: PropsWithChildren) {
  ...
  return <motion.div animate={offset} transition={{ type: 'spring', stiffness: 210, damping: 15, mass: .2 }} onPointerMove={move} onPointerLeave={() => setOffset({ x: 0, y: 0 })} className="inline-flex">{children}</motion.div>;
}
